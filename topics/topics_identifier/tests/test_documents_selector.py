...
class DocumentsSelectorTests(TestCase):
    ...
    def test_select_documents_news(self):
        ...
        mock_news_and_comments()
        documents = select_documents(documents_types, batch_number=1)
        validate_documents(self, documents, news_content)

    def test_select_documents_comments_batch1(self):
        documents_types = "comments"
        mock_news_and_comments()
        documents = select_documents(documents_types, batch_number=1, size=test_batch_size)
        validate_documents(self, documents, comments_content[:test_batch_size])
    ...
    def test_select_documents_batch2(self):
        ...
        mock_news_and_comments()
        #Execute
        documents = select_documents(documents_types, batch_number=2, size=test_batch_size)
        #Validate
